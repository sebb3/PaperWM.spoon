...
---@alias SpaceIdx number|"next"|"prev"|"recent"|"last"|"first" a Mission Control space index
---@alias Mapping { [string]: (table | string)[]}
---@alias Window hs.window
---@alias WindowId integer
---@alias WindowFilter hs.window.filter
---@alias WindowList table<SpaceIdx, table<number, Window[]>>
...
---@field floating_windows hs.watchable
local PaperWM = {
	name = "YabaiPaperWM",
	version = "0.1",
...
function PaperWM:start()
	...
	self.window_filter:subscribe({
		...
		pcall(partial(self.windowEventHandler, self, window, event))
	end)
...
---@param space? integer
---@return SpaceIdx space that contains new window
function PaperWM:addWindow(add_window, space)
	-- check if window is already in window list
	local w = self.index_table[add_window:id()]
	if w then
		return w.space
	end

	space = space or add_window:space()
...
function PaperWM:removeWindow(remove_window, skip_new_window_focus)
	...
	-- remove window
	local entryToRemove = self.window_list[remove_index.space][remove_index.col][remove_index.row]
	self.logger.df("entryToRemove: %s", entryToRemove)
	...
	if #self.window_list[remove_index.space][remove_index.col] == 0 then
		self.logger.df("last window removed, removing column")
	...
	if #self.window_list[remove_index.space] == 0 then
		self.logger.df("space empty, removing")
...
function PaperWM:windowEventHandler(window, event)
	if not coroutine.isyieldable() then
		coroutine.wrap(function()
			local status, err = pcall(partial(self.windowEventHandler, self, window, event))
			if not status then
				self.logger.df("error: %s", hs.inspect(err))
			end
		end)()
		return
	end

	self.logger.df("%s for [%s] id: %d", event, window, window:id())
	...
	elseif event == "windowDestroyed" then
		...
		self.window_filter:getWindows()[1]:focus()
		return
...
function PaperWM:tileSpace(space)
	assert(space)
	if not coroutine.isyieldable() then
		InnerWrap(self.tileSpace, self, space)()
		return
	end
	...
	local anchor_window = (function()
		if focused_window and not self.is_floating[focused_window:id()] and focused_window:space() == space then
			...
			or
			hs.window.get(self.Yabai:getSpaceAsync(space).windows[1].id)
		end
	end)()
	...
	if not anchor_index then
		...
		self.logger.e("anchor index not found. Restarting.")
		self:stop()
		self:start()
		return -- bail
	end
...
function PaperWM:focusWindow(direction, focused_index)
		...
		if focused_window:id() == 0 then
			self.logger.e("focused window not found. Focusing first visible window")
			self.window_filter:getWindows()[1]:focus()
			return
		end
	...
	elseif direction == Direction.UP or direction == Direction.DOWN then
		-- new_focused_window = self:getWindow(focused_index.space, focused_index.col, focused_index.row + (direction // 2))

		if not new_focused_window then
			local candidateWindows = {}
			if direction == Direction.DOWN then
				candidateWindows = focused_window:windowsToSouth()
			end
			if direction == Direction.UP then
				candidateWindows = focused_window:windowsToNorth()
			end
...
function PaperWM:swapWindows(direction)
	...
	local target_window
 if direction == Direction.UP then
				target_window = focused_window:windowsToNorth()[1]
				else
				target_window = focused_window:windowsToSouth()[1]
 end
 if target_window:space() ~= focused_window:space() then
     self:moveWindowToSpace(target_window:space())
     return
 end
		-- get target window
		local target_index = {
...
function PaperWM:cycleWindowSize(direction, cycle_direction)
	...
	local function findNewSize(area_size, frame_size, _cycle_direction)
		local sizes = {}
		local new_size = nil
		if _cycle_direction == Direction.ASCENDING then
		...
		elseif _cycle_direction == Direction.DESCENDING then
			for index, ratio in ipairs(self.window_ratios) do
				sizes[index] = ratio * (area_size + self.window_gap) - self.window_gap
			end
...
---@param index number|"next"|"prev" space index
function PaperWM:moveWindowToSpace(index)
	self.logger.ef("index: %s", index)
	if not coroutine.isyieldable() then
		InnerWrap(self.moveWindowToSpace, self, index)()
		return
	end
	...
	if type(index) == "string" then
		index = self.Yabai:getSpaceAsync(index).index
	end

	local old_space = self:removeWindow(focused_window)
	self:addWindow(focused_window, index)
	self:tileSpace(index)
	self:tileSpace(old_space)
	-- self.Yabai:moveToSpace(index, focused_window)
...
function PaperWM:toggleFloating(window, value)
	...
	if not coroutine.isyieldable() then
		PartialWrap(self.toggleFloating, self, window, value)()
		return
	end
	...
	if value == nil then
		value = not self.is_floating[id] or nil
	end

	self.is_floating[id] = value
	self:persistFloatingList()

	local space = (function()
		if self.is_floating[id] then
			return self:removeWindow(window, false)
		else
...
function PaperWM:toggleConsole()
	if not coroutine.isyieldable() then
		PartialWrap(self.toggleConsole, self)()
		return
	end
	...
	local children = ax:allDescendantElementsAsync()
	inputs = hs.fnutils.filter(children, function(el)
		return el.role and el:role() == "AXTextField"
	end)
...
PaperWM.actions = {
	...
	focus_left = InnerWrap(PaperWM.focusWindow, PaperWM, Direction.LEFT),
	focus_right = InnerWrap(PaperWM.focusWindow, PaperWM, Direction.RIGHT),
	focus_up = InnerWrap(PaperWM.focusWindow, PaperWM, Direction.UP),
	focus_down = InnerWrap(PaperWM.focusWindow, PaperWM, Direction.DOWN),
	swap_left = InnerWrap(PaperWM.swapWindows, PaperWM, Direction.LEFT),
	swap_right = InnerWrap(PaperWM.swapWindows, PaperWM, Direction.RIGHT),
	swap_up = InnerWrap(PaperWM.swapWindows, PaperWM, Direction.UP),
	swap_down = InnerWrap(PaperWM.swapWindows, PaperWM, Direction.DOWN),
	center_window = InnerWrap(partial(PaperWM.centerWindow, PaperWM)),
	...
	switch_space_l = InnerWrap(partial(PaperWM.switchToSpace, PaperWM, "prev")),
	switch_space_r = InnerWrap(partial(PaperWM.switchToSpace, PaperWM, "next")),
	switch_space_1 = InnerWrap(partial(PaperWM.switchToSpace, PaperWM, 1)),
	switch_space_2 = InnerWrap(partial(PaperWM.switchToSpace, PaperWM, 2)),
	switch_space_3 = InnerWrap(partial(PaperWM.switchToSpace, PaperWM, 3)),
	switch_space_4 = InnerWrap(partial(PaperWM.switchToSpace, PaperWM, 4)),
	switch_space_5 = InnerWrap(partial(PaperWM.switchToSpace, PaperWM, 5)),
	switch_space_6 = InnerWrap(partial(PaperWM.switchToSpace, PaperWM, 6)),
	switch_space_7 = InnerWrap(partial(PaperWM.switchToSpace, PaperWM, 7)),
	switch_space_8 = InnerWrap(partial(PaperWM.switchToSpace, PaperWM, 8)),
	switch_space_9 = InnerWrap(partial(PaperWM.switchToSpace, PaperWM, 9)),
